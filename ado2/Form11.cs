...
namespace ado2
{
    public partial class Form11 : Form
    {
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please enter the User ID of the record you want to delete.", "Input Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (result == DialogResult.Yes)
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    try
                    {
                        ...
                        string deleteQuery = "DELETE FROM hod WHERE userid = @userid"; 
                        using (SqlCommand deleteCmd = new SqlCommand(deleteQuery, conn))
                        {
                            ...
                            {
                                MessageBox.Show("No record found with the specified User ID.", "Delete Failed", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            }
                        }
                    }
                    catch (Exception ex)
